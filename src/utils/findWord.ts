export function findWord(targetWord: string, lettersKit: string[], vocabulary: string[]) {
  ...
  for (let i = 0; i < targetWord.length; i++) {
    ...
    else targetWordMask.push(`[${[...new Set(letters)].toLocaleString().replaceAll(',', '')}]`)
  }

  const targetWordMaskPrepared = new RegExp('^' + targetWordMask.toLocaleString().replaceAll(',', '') + '$')

  const findWords = () => {
    const targetWords = vocabulary.filter((item) => targetWordMaskPrepared.test(item))

    const targetWordsFilteredDuplicatedTargetLetters = targetWords.filter((word) => {
      let shouldBeFiltered = 0

      word.split('').forEach((letter) => {
        //@ts-ignore
        if (howManyRepeats(word, letter) > duplicatedLettersMap[letter]) shouldBeFiltered += 1
      })
